"""Manned walls: paired host-browser benchmark. Not a physical-phone/battery test.
...
p=argparse.ArgumentParser();p.add_argument('--baseline',required=True);p.add_argument('--seconds',type=float,default=20);p.add_argument('--exercise',action='store_true');p.add_argument('--output',default='verification-output/performance-walls');a=p.parse_args()
...
with sync_playwright() as pw:
 ...
 for scenario in (['exercise'] if a.exercise else ['overview','tower']):
  for variant in (['walls','walls'] if a.exercise else ['baseline','walls','walls','baseline']):
   ...
   page.evaluate('''()=>{Object.assign(estado,{trechosPalicadaInterna:4,trechosPalicadaExterna:4,trechosMuralhaPedraInterna:4,trechosMuralhaPedraExterna:4,quantidadeTorresMuralha:8,quantidadePortoesFortificados:7,arqueirosMilicia:4,arqueirosGuarda:4,arqueirosSoldados:4,estoqueFlechas:100,estoqueArcos:15});if(typeof normalizarDefesaPosicional==='function'){estado.trechosAdarveExterno=4;estado.trechosAdarveInterno=4;estado.defesaPosicional=normalizarDefesaPosicional(null);estado.jogoPausado=false;atualizarGuarnicaoColonia(500)}}''')
   ...
   page.evaluate('centerTest(7000,4500,.1)' if scenario=='overview' else 'centerTest(5200,1700,.6)')
   if scenario=='exercise':
    page.evaluate('''()=>{const original=atualizarGuarnicaoColonia;atualizarGuarnicaoColonia=function(delta){if(!estado.defesaPosicional.exercicio){iniciarExercicioMuralhas();centerTest(4600,1120,.6)}return original(delta)}}''')
