...
function game(html,storage=new Map()){
 const outer={require:n=>(n==='fs'||n==='node:fs')?{...fs,readFileSync:(p,...args)=>p==='index.html'?html:fs.readFileSync(p,...args)}:require(n),Buffer,console,TextEncoder,TextDecoder,URL,Blob,Date:FixedDate};
...
for(const scenario of ['mature','shortage','hunt']){
 ...
 assert.equal(a.eval('larguraMundoExpansao'),14000,'Historical source actually loaded');assert.equal(b.eval('larguraMundoExpansao'),17500,'Current source actually loaded');
